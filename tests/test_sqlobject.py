...
sys.path.insert(0, '/home/cahnm/python/SQLObject-0.6.1') #@@ MHC
...
class Counter(SQLObject):
    ...
    _columns = [
        IntCol('some_number', notNull=True),
        ]
...
class SliceTest(SQLObjectTest):
    ...
    def inserts(self):
        for i in range(100):
            Counter(some_number=i)

    def counterEqual(self, counters, value):
        self.assertEquals([c.some_number for c in counters], value)

    def test1(self):
        self.counterEqual(Counter.select('all', orderBy='some_number'), range(100))

    def test2(self):
        self.counterEqual(Counter.select('all', orderBy='some_number')[10:20],
                          range(10, 20))

    def test3(self):
        self.counterEqual(Counter.select('all', orderBy='some_number')[20:30][:5],
                          range(20, 25))

    def test4(self):
        self.counterEqual(Counter.select('all', orderBy='some_number')[:-10],
                          range(0, 90))

    def test5(self):
        self.counterEqual(Counter.select('all', orderBy='some_number', reversed=True), range(99, -1, -1))

    def test6(self):
        self.counterEqual(Counter.select('all', orderBy='-some_number'), range(99, -1, -1))
...
class SOIndex1(SQLObject):
    ...
    nameIndex2 = DatabaseIndex(name, some_number)
    # MHC @@ Oracle can't index the same column twice.  (The length
    # argument is ignored.)
...
class IndexTest1(SQLObjectTest):
    ...
    def test(self):
        ...
        for name in 'blah blech boring yep yort snort'.split():
            n += 1
            SOIndex1(name=name, some_number=n)
        mod = SOIndex1._connection.module

        try:
            SOIndex1(name='blah', some_number=0)
