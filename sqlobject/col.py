...
class SOStringCol(SOCol):
    ...
    def _oracleType(self):
        # What do we want when no length is specified?  You can only have one LONG column in an Oracle table.
        if not self.length:
            return 'CHAR(2000 BYTE)'
        else:
            return self._sqlType()
...
class SOBoolCol(SOCol):
    ...
    def _oracleType(self):
        # Oracle's BOOLEAN data type isn't used for columns, only for variables.
        return "NUMBER(1,0)"
...
class SOForeignKey(SOKeyCol):
    ...
    def oracleCreateSQL(self):
        ...
        if self.cascade is not None:
            ...
            action = ''
        # Don't try to name the constraint.  If two tables contain the same foreign key,
        # a duplicate constraint name error will result.
        constraint = ('REFERENCES %(tName)s (%(idName)s) '
                      '%(action)s' %
                      {'tName': tName,
                       'colName': self.dbName,
                       'idName': idName,
...
class SODateCol(SOCol):
    ...
    def _oracleType(self):
        # This is both a date and time, the same as the DateTimeCol
        return 'DATE'
...
class BinaryValidator(validators.Validator):
    ...
    def toPython(self, value, state):
        return state.soObject._connection.returnBinary(value)
